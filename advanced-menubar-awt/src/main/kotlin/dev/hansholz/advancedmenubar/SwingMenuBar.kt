...
internal fun swingMenuBarFromModel(menus: List<TopMenu>): JMenuBar =
    JMenuBar().apply {
        menus.forEach { top ->
            when (top) {
...
private fun JMenu.addSwingElements(elements: List<MenuElement>) {
    elements.collapseSeparators().forEach { el ->
        when (el) {
            ...
            is FormatStd.AlignJustified ->
                add(
                    swingStdCheckboxItem(el.title, el.checked, el.enabled, icon = el.icon, onToggle = el.onToggle),
                )
...
private fun MenuIcon.toSwingIcon(): Icon? =
    synchronized(swingIconCache) {
